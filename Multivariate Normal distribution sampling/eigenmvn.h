...
namespace Eigen {
	namespace internal {
		...
		template<typename Scalar>
		mxws<uint32_t> scalar_normal_dist_op<Scalar>::rng;
	...
	public:
		...
		void setCovar(const Matrix<Scalar, Dynamic, Dynamic>& covar)
		{
			...
			if (_use_cholesky)
			{
				...
				{
					std::cout << "Failed computing the Cholesky decomposition. Use solver instead" <<
						std::endl;
					exit(-1);
				}
			}
...
/// as columns in a Dynamic by nn matrix
		Matrix<Scalar, Dynamic, -1> samples(const uint64_t& nn)
		{
			return (_transform * 
				Matrix<Scalar, Dynamic, -1>::NullaryExpr(_covar.rows(), nn, randN)).colwise() 
				+ _mean;
		}
