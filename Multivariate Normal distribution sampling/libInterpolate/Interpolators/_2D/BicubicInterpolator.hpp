...
namespace _2D {
		...
		BicubicInterpolator<Real>::setupInterpolator()
	{
		...
		for (auto i = 0; i < X->size() - 1; i++)
		{
			for (auto j = 0; j < Y->size() - 1; j++)
			{
				...
				ip = std::min(i + 1ULL, iN - 1ULL); // get index to the "right" of current i
		...
		BicubicInterpolator<Real>::operator()(Real x, Real y) const
	{
		...
		auto i = this->get_x_index_to_left_of(x);
		auto j = this->get_y_index_below(y);
